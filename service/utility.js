...
class UtilityService extends RestExternalService {
	async initialize(correlationId) {
		try {
			const response = await this._serviceCommunicationRest.get(correlationId, ClientConstants.ExternalKeys.BACKEND, 'utility/initialize');
	...
	async logger(correlationId, content) {
		...
		return await this._serviceCommunicationRest.post(correlationId, ClientConstants.ExternalKeys.BACKEND, 'utility/logger', content);
	}

	async openSource(correlationId) {
		try {
			const response = await this._serviceCommunicationRest.get(correlationId, ClientConstants.ExternalKeys.BACKEND, 'utility/openSource');
			this._logger.debug('ApiService', 'initialize', 'response', response, correlationId);
			return response;
		}
		catch (err) {
